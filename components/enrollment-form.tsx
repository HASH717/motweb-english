...
function SubmitButton({ label }: { label: string }) {
  ...
  return <button disabled={pending} className="focus-ring inline-flex items-center justify-center gap-2 rounded-[var(--radius)] bg-[var(--color-accent)] px-5 py-4 text-base font-black text-white shadow-lg shadow-[#32a81f]/25 transition hover:bg-[#3ac724] disabled:opacity-70">
    ...
    {pending ? "Preparing offers..." : label}
  </button>;
}

type EnrollmentFormProps = {
  itemType?: "canva" | "ebook";
  itemSlug?: string;
  eyebrow?: string;
  title?: string;
  description?: string;
...
export function EnrollmentForm({
  ...
  title = "Enter your Canva details",
  description = "We use this email to send your Canva Pro invitation after your offer completion is confirmed.",
  emailLabel = "Your Canva email",
  buttonLabel = "Continue to free activation"
}: EnrollmentFormProps) {
  ...
  return <form action={action} className="surface order-2 rounded-[var(--radius)] p-5 sm:p-7 lg:col-start-2 lg:row-span-2 lg:row-start-1">
    ...
    <p className="text-sm font-bold text-[#cf19dd]">{eyebrow}</p>
    ...
    <div className="mt-6 grid gap-5">
      ...
      <label className="grid gap-2"><span className="text-sm font-bold text-white">{emailLabel}</span><input required name="email" type="email" autoComplete="email" dir="ltr" className="focus-ring rounded-[var(--radius)] border border-[var(--color-input-border)] bg-[var(--color-input-background)] px-4 py-3 text-white" placeholder="name@example.com" /></label>
